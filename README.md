...
- 1. **왜 검색 API 에 Cache 를 적용했는지 스스로 꼭!! 고민하고 Readme 작성.**
...
- 3. Redis 는 대표적인 Key-Value 형태로 데이터를 저장하는 대표적인 NoSQL 저장소다.
...
- 4. **Redis Cache 를 적용한 검색 API 에 대해서 v1, v2 API 각각 성능테스트 (nGrinder)**
...
### **6. Redis Sorted Set 백업 스케줄러**

- 1. **인기 검색어 백업 스케줄러** (`PopularKeywordScheduler`)
    - **목적**
...
- 2. **영화 랭킹 백업 스케줄러** (`MovieRankingScheduler`)
    - **목적**
    ...
    - **동작**
        1. **일간 백업**
            ...
            2. Redis의 '어제' 날짜 일간 랭킹 Sorted Set(`movie_scores:daily:...`)을 
            
            `reverseRangeWithScores` 명령어를 이용하여 100위까지 조회
            3. RDBMS의 `movie_rankings` 테이블에 저장
        2. **주간 백업**
            1. 매주 **월요일** 새벽 4시 20분에 `@Scheduled`에 의해 실행
            2. Redis의 주간 랭킹 Sorted Set(`movie_scores:weekly:...`)을 
